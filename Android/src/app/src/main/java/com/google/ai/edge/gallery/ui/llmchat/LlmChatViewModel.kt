...
open class LlmChatViewModelBase() : ChatViewModel() {
  fun generateResponse(
    ...
    viewModelScope.launch(Dispatchers.Default) {
      ...
      var initialPrefillTokens = images.size * 257
      ...
      for (audioMessage in audioMessages) {
        ...
        initialPrefillTokens += (duration * 1000f / 150f).toInt()
      }

      val tracker = LlmBenchmarkTracker(initialPrefillTokens = initialPrefillTokens)
      ...
      try {
        LlmChatModelHelper.runInference(
          ...
          resultListener = { partialResult, done ->
            ...
            val latencyMs: Float =
              if (done) tracker.computeLatencySeconds() * 1000f else -1f
